...
* nspk template parameters: 1=language name (other script for now, non-localizable), 2=pre-comment (e.g. synonym, localizable).
  * If there’s no {{nspk}} in languages and there’s language data, default {{nspk}} is added. So: {{nspk}} at the end → delete! Need e.g. synonym → add {{nspk||=Klingon}}. Synonym is the SECOND parameter.
