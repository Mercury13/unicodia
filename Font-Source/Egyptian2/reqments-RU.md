# Требования к египетским тероглифам
...
Избегай геометрических объектов, они обычно нарисованы как надо. Желательно взять набор дыр (например, стоящие женщины) и устроить кампанию по закрытию этих дыр.
...
Рисовать надо ровно то, что требуют изображение и описание. Если изображение и описания расходятся — 1) не рисуй; 2) спроси у меня; 3) посмотри, на чьей стороне шрифт NewGardiner.

Конечности зачернены, кроме особых случаев.

Стандартная короткая причёска и форма бороды нее указана — значит, без бороды.
...
Жёлтый — жёлтым мы закрашиваем то, что должно стать пустотой. При экспорте вычесть.
