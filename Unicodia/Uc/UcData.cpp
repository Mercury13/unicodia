...
constexpr auto CT_CONTROL =
    u8"В первые годы, когда часто требовалось повторить сессию обмена вручную (перфоратором или консолью), "
            "управляющие символы играли роль форматов данных и сетевых протоколов. "sv
        "В настоящее время используются немногие из них:<br>"
        "• нулевой символ 00;<br>"
        "• конец файла 04 (Unix) и 1A (DOS/Windows);<br>"
        "• шаг назад (забой) 08;<br>"
        "• табуляция 09;<br>"
        "• перевод строки 0A;<br>"
        "• прогон листа 0C;<br>"
        "• возврат каретки 0D;<br>"
        "• Escape 1B."sv;
...
constinit const uc::Term uc::terms[] {
      ...
      u8"письменность спорного типа"sv, {},
        u8"<p>Для двух письменностей источники не договорились, что они собой представляют."
            ...
            "<p>[[ps:Thaa|Тана]] ''(мальдивский)''{{-}}[[pt:abjad|консонантная]] ''(Omniglot)'', "
                    "[[pt:abugida|абугида]] ''(английская Википедия)'' или алфавитная ''(Юникод)''."sv },
      ...
      u8"UTF-8"sv, {},
        u8"<p>'''Unicode transformation format, 8-bit'''{{-}}формат [[pt:serialization|сериализации]] Юникода в цепочку байтов. "
            ...
            "<p>Свойства UTF-8:<br>"
                ...
                "• Монотоный: программа, не понимающая UTF-8, отсортирует тексты в порядке кодов."
      ...
      u8"UTF-16"sv, {},
        u8"<p>'''Unicode transformation format, 16-bit'''{{-}}формат [[pt:serialization|сериализации]] Юникода в цепочку двухбайтовых слов. "
            ...
            "<p>Свойства UTF-16:<br>"
                "• Накладывает ограничения на ёмкость Юникода{{-}}2<big>²⁰</big>+2<big>¹⁶</big>−2<big>¹¹</big>=1.112.064 символов.<br>"
                "• Обратная совместимость: текст в [[pt:bmp|базовой плоскости]] без пользовательских символов неотличим от UCS-2. "
                    "Если рабочий язык лежит в базовой плоскости, а другие символы остаются или удаляются, "
            ...
            "<p>Как формат межпрограммного обмена используется в основном в двоичных файлах родом с Windows. "
                "Таким файлам [[pt:bom|метка порядка байтов]] не нужна; в текстовых файлах она "
                    "FE{{_}}FF для UTF-16BE и FF{{_}}FE для UTF-16LE. "
                "Как формат программного интерфейса используется в Windows, Java, Qt."sv },
    { "utf32", TermCat::SERIALIZATION,
      u8"UTF-32"sv, {},
        u8"<p>'''Unicode transformation format, 32-bit''', он же '''UCS-4'''{{-}}"
                    "формат [[pt:serialization|сериализации]] Юникода в цепочку четырёхбайтовых слов. "
    ...
    { "utf7", TermCat::SERIALIZATION,
      u8"UTF-7"sv, {},
        u8"<p>'''Unicode transformation format, 7-bit'''{{-}}"
            ...
            "<p>Если после кода идёт любой символ, кроме символов Base64 и дефиса, "
                    "заключительный дефис тожно опустить."
    ...
    { "punycode", TermCat::SERIALIZATION,
      u8"Punycode"sv, {},
    ...
    { "bom", TermCat::SERIALIZATION,
      u8"метка порядка байтов"sv, u8"byte order mark"sv,
        u8"<p>Символ U+FEFF. Его можно (не обязательно) ставить в начале текстового файла, чтобы указать его формат. "
                ...
                "• 00{{_}}00{{_}}FE{{_}}FF{{-}}[[pt:utf32|UTF-32BE]] (как формат текстовых файлов встречается крайне редко);<br>"
                "• FF{{_}}FE{{_}}00{{_}}00{{-}}UTF-32LE  (как формат текстовый файлов встречается крайне редко)."
            "<p>[[pt:utf7|UTF-7]] и [[pt:punycode|Punycode]] не встречаются как формат текстовых файлов."sv },
};
