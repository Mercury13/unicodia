...
const uc::Script uc::scriptInfo[static_cast<int>(uc::EcScript::NN)] {
    ...
    { "Hani"sv, EcScriptType::HIEROGLYPH, EcLangLife::ALIVE, EcWritingDir::LTR_CJK,
        ...
        u8"<p>Китайское письмо (ханьцзы) используется уже несколько тысячелетий, а первые пиктограммы относятся к VI\u00A0тысячелетию до н.э. "sv
                u8"В III веке иероглифы проникли в Японию.</p>"sv
            u8"<p>В Китае, Тайване, Японии и Корее иероглифы незначительно отличаются, Юникод эти различия не кодирует, "sv
                u8"объединяя их все позиции в один блок CJK (<i>Chinese, Japanese, Korean</i>).</p>"sv
