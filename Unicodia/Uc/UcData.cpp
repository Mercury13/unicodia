...
constinit const uc::ScriptType uc::scriptTypeInfo[static_cast<int>(EcScriptType::NN)] {
    ...
    { u8"[[pop_term:syllabic|слоговая]]{{-}}''как японские катакана/хирагана''"sv },
    { u8"алфавитно-слоговая <i>(как иберийский)</i>"sv },
    { u8"[[pop_term:abugida|абугида]]{{-}}''как эфиопский''"sv },
    { u8"[[pop_term:brahmic|брахмийская абугида]]{{-}}''как деванагари''"sv },
...
constinit const uc::Category uc::categoryInfo[static_cast<int>(uc::EcCategory::NN)] {
    ...
    { UpCategory::LETTER,       "Lo"sv,     u8"Буква/другая"sv,
            ...
            u8"Например, [[pop_scr:Hebr|иврит]], [[pop_scr:Arab|арабская вязь]] и [[pop_scr:Hani|китайские иероглифы]]. Также особые символы вроде "
    ...
    { UpCategory::SYMBOL,       "So"sv,     u8"Символ/другой"sv,
            u8"Юникод содержит множество разных символов, в том числе коммерческие, технические и [[pop_term:emoji|эмодзи]]."sv },
    ...
    { UpCategory::SEPARATOR,    "Zs"sv,     u8"Разделитель/пробел"sv,
            u8"В хорошо проработанной типографике "
                "пустого места столько, сколько нужно: 146%{{-}}мало, 146{{_}}%{{-}}много, "
                "{{%|146}}{{-}}самое то. Потому и пробелы бывают разные. "
                "Некоторые из них неразрывные: не расширяются при выключке, не переносятся на другую строку."
...
constinit const uc::Script uc::scriptInfo[] {
    ...
    { "Beng"sv, QFontDatabase::Bengali,
        ...
        u8"<p>Относится к восточной ветви индийского письма. Гласная по умолчанию{{-}}среднее между «а» и «о», "
            ...
            "<p>[[pop_term:virama|Вирама]] (''хосонто'', убрать гласную по умолчанию) используется крайне редко{{-}}{{sm|নাক্ }}= нôк{{-}}"
    ...
    { "Deva"sv, QFontDatabase::Devanagari,
        ...
        u8"<p>Деванагари (буквально «язык божественного города») развился из письма брахми и стал алфавитом для многих языков Индии. "
            ...
            "<p>Согласные буквы означают слог с гласной «а». Чтобы отобразить другие слоги, надо добавить огласовку: "
                ...
                "Чтобы получить простую согласную, надо добавить знак «[[pop_term:virama|вирама]]» («остановка»): к{{_}}{{sm|क्}}.</p>"
            "<p>Вирамы в деванагари редки и обычно встречаются в конце слов из-за [[pop_term:ligature|лигатур]]: "
    ...
    { "Dogr"sv, QFontDatabase::Any,
        ...
        u8"<p>Язык догров, индийской народности. Письменность позаимствована из [[pop_scr:Takr|такри]], стандартизирована "
            ...
            "<p>Перед нами брахмийская абугида с гласной по умолчанию «ə» (в Юникоде условно названа «а»), "
                ...
                "К тому же в догри встречаются [[pop_scr:ligature|лигатуры]], не закодированные в данном шрифте. "
    ...
    { "Mong"sv, QFontDatabase::Any,
        ...
        u8"монгольский"sv,
        u8"<p>В 1204 монголы завоевали уйгуров и позаимствовали алфавит, восходящий к [[pop_scr:Syrc|сирийскому]]. "
            ...
            "<p>В России письменность использовалась калмыками и бурятами. "
                "После войны монголы разработали новую письменность на [[pop_scr:Cyrl|кириллице]], но с развалом СССР есть планы "
...
constinit const uc::Block uc::blocks[302] {
    ...
    { 0x20D0, 0x20FF,
            ...
            u8"Практически все из этих символов нормально работают только если в шрифте есть подходящая [[pop_term:ligature|лигатура]]{{-}}"
    ...
    { 0x2440, 0x245F,
            ...
            u8"<p>Оптическое и магнитное распознавание символов служит для автоматического ввода документов, "
                ...
                "<p>В 1968 в США сделали первую действующую технологию оптического распознавания{{-}}OCR-A. "
                    "Понимает весь [[pop_scr:Latn|латинский]] алфавит с малыми буквами. "
    ...
    { 0x2460, 0x24FF,
            ...
            u8"По данным английской Википедии, эти символы в основном предназначены для списков{{-}}"
                    ...
                    "{{sm|Ⓜ}} используется как [[pop_term:emoji|эмодзи]] для метро."sv },
    // Box drawing OK
    { 0x2500, 0x257F,
            "Box Drawing", u8"Рисование рамок"sv,
            u8"<p>Элементы [[pop_term:pseudographics|псевдографики]], используемые для рисования рамок. "
    ...
    { 0x2580, 0x259F,
            ...
            u8"<p>Элементы [[pop_term:pseudographics|псевдографики]], используемые для сплошной заливки. "
                    "Как и любая псевдографика, чаще всего используются в моноширинных шрифтах. "
    ...
    { 0x25A0, 0x25FF,
            ...
            u8"<p>Геометрические фигуры используются для маркировки кнопок, грубого рисования пользовательских интерфейсов. "
                    ...
                    "Символ {{sm|◌}} используется в Юникоде для маркировки [[pop_term:combining|комбинирующих]] знаков."sv },
    // Misc sym OK
    { 0x2600, 0x26FF,
            "Miscellaneous Symbols", u8"Разные символы"sv,
            u8"<p>В этом разделе есть [[pop_term:emoji|эмодзи]], знаки из астрономии, астрологии, настольных игр, восточной эзотерики, "
    ...
    { 0x2700, 0x27BF,
            ...
            u8"<p>Символы, чаще всего используемые для украшения печатного текста. "
                    "Блок содержат несколько [[pop_term:emoji|эмодзи]], что вполне оправдано: в интернете это будут цветные картинки, "
    ...
    { 0xFB00, 0xFB4F,
            ...
            u8"<p>Стандартные [[pop_term:ligature|лигатуры]] для [[pop_scr:Latn|латиницы]], [[pop_scr:Armn|армянского]] и [[pop_scr:Hebr|иврита]]."sv },
    { 0xFB50, 0xFDFF,
    ...
    { 0xFE00, 0xFE0F,
            ...
            u8"<p>Чаще всего (например, в браузерах) тот или иной вариант написания символа включают, выбирая другой шрифт "
                ...
                "<p>[[pop_scr:Latn|Латиница]], [[pop_scr:Mymr|бирманский и ещё несколько письменностей используют VS1. "
                    "[[pop_scr|Hani|Китайские иероглифы]] используют VS1…VS3. [[pop_term:emoji|Эмодзи]] можно сделать цветными (VS16) "
            ...
            u8"<p>С X{{_}}века появилась нотная запись, похожая на современную. "
                ...
                "<p>Юникод как таковой не умеет писать музыку. "
                    ...
                    "В 2013 вышел открытый стандарт ''Standard Music Font Layout'' (SMuFL), расширяющий блок "
                        "до более чем 2600 символов с использованием позиций для личного пользования."
    ...
    { 0xE0100, 0xE01EF,
            ...
            u8"Дополнительные селекторы начертания используются только с [[pop_scr:Hani|китайскими иероглифами]], чтобы дать тот или иной "
                "вариант начертания."sv },
    //{ 0xE0100, 0xE01EF, "Check for error"sv },
};

constinit const uc::Term uc::terms[] {
    { "syllabic", TermCat::SCRIPT_CLASS,
      u8"слоговая азбука"sv, u8"syllabic script"sv,
        u8"<p>Письменность, в которой каждая буква записывает слог из гласной и согласной. "
                ...
                "Разновидностью слоговой письменности является [[pop_term:abugida|абугида]], где у сходных слогов сходные буквы."
            "<p>Слоговую письменность легко изобрести из иероглифической: люди говорят не звуками, а слогами, "
                ...
                "Подобный процесс замечен в [[pop_scr:Xsux|клинописи]], [[pop_scr:Egyp|египетских иероглифах]], [[pop_scr:Hira|японском]]."
            "<p>Чтобы записывать стечения и одинокие согласные, есть несколько вариантов (возьмём для примера слово ''стол''):<br>"
                "• придумать символ для одинокой буквы: с+то+л ([[pop_scr:Cans|канадская слоговая]]);<br>"
                "• сделать такой символ, приделав к слогу [[pop_scr:virama|умляут]]: сǎ+то+лǎ "
                    "(многие [[pop_term:brahmic|брахмийские абугиды]], особенно малоразвитые);<br>"
                "• запретить стечения и конечные: су+то+лу (японский, особенно в заимствованиях);<br>"
                "• в определённых условиях гласная действительно бывает немой: сᵊ+то+лᵊ ([[pop_scr:Guru|гурмукхи]], частично японский);<br>"
                "• придумать [[pop_term:ligature|лигатуры]]: сᵀо+?? (развитые брахмийские абугиды вроде [[pop_scr:Deva|деванагари]]);<br>"
                "• отбросить конечную согласную: ??+то ([[pop_scr:Linb|микенский]], [[pop_scr:Buhd|бухид]])."sv },
    { "abugida", TermCat::SCRIPT_CLASS,
      u8"абугида"sv, u8"abugida"sv,
        u8"<p>Исследователи не сходятся в определении абугиды, и самое широкое из них такое: "
                    "[[pop_term:syllabic|слоговая]] азбука, где у сходных слогов сходные символы."
            "<p>Абугидами '''точно являются''' [[pop_term:brahmic|брахмийские письменности]]."
            "<p>Абугидами '''являются у многих исследователей''' [[pop_scr:Ethi|эфиопская]] и [[pop_scr:Cans|канадская слоговая]] "
                    "письменности."
            "<p>Абугидами '''не являются ни в одном источнике''' [[pop_scr:Hang|хангыль]], [[pop_scr:Thaa|тана]], [[pop_scr:Yiii|и]]."
    ...
    { "brahmic", TermCat::SCRIPT_CLASS,
      u8"брахмийская абугида"sv, u8"Brahmic abugida"sv,
        u8"<p>Исследователи расходятся в определении [[pop_term:abugida|абугиды]]. "
                "Но в одном они единогласны: абугидами является '''группа письменностей, восходящих к [[pop_scr:Brah|брахми]]'''."
            "<p>В семитских языках корнем слова являются согласные, а гласные{{-}}суффиксы и окончания: "
                    "в [[pop_scr:Arab|арабском]] ''джихадом'' занимается ''моджахед'' (или, точнее, ''муджахид''). "
                "Для них естественно было ввести [[pop_term:abjad|консонантную]] письменность. "
                "Когда [[pop_scr:Phnx|финикийское]] письмо пошло на восток и столкнулось с индоевропейскими языками, появилась "
                    "слоговая письменность, которая в том или ином виде распространилась на всю Юго-Восточную Азию, кроме "
                    "[[pop_term:cjk|Китая, Японии и Кореи]]."
            "<p>Особенности брахмийских абугид:<br>"
                "• согласная буква означает слог с некоей гласной по умолчанию, остальные приделываются огласовками: "
                    "[[pop_scr:Deva|деванагари]] <nobr>ка {{sm|क}} + и {{sm| ि}} = ки {{sm|कि}}</nobr>;<br>"
                "• чтобы получить одинокую согласную, используется знак «[[pop_term:virama|вирама]]»: "
                    "<nobr>ка {{sm|क}} + {{sm|्}} = к {{sm|क्}}</nobr>;<br>"
                "• ''могут'' быть [[pop_term:ligature|лигатуры]] из нескольких согласных{{-}}деванагари {{sm|क्ष}} ксса. "
                    "Вводятся они, как будто лигатуры нет: ка+вирама+сса;<br>"
                ...
                "• алфавитный порядок{{-}}фонологический: сначала гласные, потом согласные от глотки «к» до зубов «с», "
                    "и в конце выдох «h»."
            "<p>Брахмийскими абугидами являются большинство письменностей Индии, [[pop_scr:Khmr|кхмерский]], [[pop_scr:Thai|тайский]], "
                    "[[pop_scr:Laoo|лаосский]], [[pop_scr:Lana|ланна]], а также множество мелких исчезающих письменностей региона "
                    "вроде [[pop_scr:Sund|сунданского]]."sv },
    { "cjk", TermCat::SCRIPT_CLASS,
      u8"ККЯ = китайский, корейский, японский"sv, u8"CJK = Chinese, Japanese, Korean"sv,
        u8"<p>Группа письменностей, восходящих к [[pop_scr:Hani|китайским иероглифам]]. "
                "К ним относятся также слоговые [[pop_scr:Kana|катакана]] и [[pop_scr:Hira|хирагана]], и "
                    "[[pop_term:pseudoalphabet|псевдоалфавитный]] [[pop_scr:Hang|хангыль]]."
            "<p>Особенности ККЯ:<br>"
                "• размашистое письмо кистью и тушью;<br>"
                "• символы моноширинные{{-}}независимо от количества черт, выровнены по невидимой сетке;<br>"
                "• письмо изначально справа налево по столбцам; сейчас ''обычно'' слева направо по строкам. "
                    "И поныне мангу читают справа налево;<br>"
                "• иероглифы несколько отличаются от страны к стране, Юникод оставляет эти тонкости за шрифтами;<br>"
                "• поскольку символы не вписываются в 256 [[pop_term:codepoint|позиций]], существовали специфичные кодировки для иероглифов."sv },
    { "ligature", TermCat::CHAR_CLASS,
      u8"лигатура"sv, u8"ligature"sv,
        u8"<p>Лигатура{{-}}это несколько символов в одном:"
            ...
            "<p>2. В шрифтовом деле{{-}}символ, означающий последовательность из нескольких [[pop_term:codepoint|кодовых позиций]]. "
                "Так, в некоторых шрифтах набор «fi» пропечатывает лигатуру '''ﬁ'''. "
                "Много таких лигатур в [[pop_term:brahmic|брахмийских абугидах]]."sv },
    { "virama", TermCat::CHAR_CLASS,
      u8"вирама"sv, u8"virama"sv,
        u8"<p>Вирама (санскрит «остановка»){{-}}знак в [[pop_term:brahmic|брахмийских]] абугидах, означающий «стереть гласную по умолчанию». "
                "Например, в [[pop_scr:Deva|деванагари]]: <nobr>ка {{sm|क}} + {{sm|्}} = к {{sm|क्}}</nobr>."
            ...
            "<p>Некоторые языки пользуются вирамой нечасто. Например:<br>"
                "• используют [[pop_term:ligature|лигатуры]]{{-}}деванагари {{sm|क्ष}} кса. "
                    "Вводятся они, как будто лигатуры нет: ка+вирама+са;<br>"
                "• гласная по умолчанию редуцировалась{{-}}[[pop_scr:Guru|пенджабский]];<br>"
                "• реформа орфографии лишила букву гласной по умолчанию{{-}}[[pop_scr:Laoo|лаосский]];<br>"
                "• вирама может быть единым целым с согласной{{-}}[[pop_scr:Talu|новое письмо лы]]."sv },
    { "emoji", TermCat::CHAR_CLASS,
      u8"эмодзи"sv, u8"emoji"sv,
        u8"<p>От японского {{sm|絵}} ''э{{-}}картинка'' + {{sm|文字}} ''модзи{{-}}знак, символ."
            "<p>Придуманы мобильным оператором NTT DoCoMo около 1999 для экономии места в SMS. "
                ...
                "Другие изменили значение за пределами Японии: знак высшего балла {{em|💯}} превратился в «полностью согласен»."sv },
    { "codepoint", TermCat::ENCODING,
      u8"кодовая позиция"sv, u8"code point"sv,
        u8"<p>Кодовая позиция{{-}}число или последовательность чисел, которые представляют некую концепцию "
                    "в кодировке{{-}}символ, управляющую команду, [[pop_term:combining|комбинирующую метку]]. "
                "Или теоретически могут представлять, но пусты{{-}}временно или навечно. "
            ...
            "<p>В Юникоде кодовая позиция записывается числом от 0 до 1114111 (в шестнадцатеричной системе{{-}}до 10FFFF). "
                "Не путать с ''кодовыми единицами''{{-}}байтами или словами в "
                    "[[pop_term:utf8|UTF-8]]/[[pop_term:utf16|16]]/[[pop_term:utf32|32]]."
            "<p>Один символ может записываться в разные кодовые позиции, потому что изображает разные концепции. "
                "Простейший пример{{-}}A [[pop_scr:Latn|латинское]] и [[pop_scr:Cyrl|русское]]."sv },
    { "serialization", TermCat::SERIALIZATION,
      u8"сериализация"sv, u8"serialization"sv,
        u8"<p>Метод преобразования данных в цепочку ''кодовых единиц''{{-}}байтов для [[pop_term:utf8|UTF-8]], "
                    "слов из двух байтов для [[pop_term:utf16|UTF-16]], слов из четырёх байтов для [[pop_term:utf32|UTF-32]]."
            "<p>Поскольку диски и каналы связи оперируют байтами, при передаче UTF-16/32 играет роль порядок байтов в слове. "
                "Соответственно появляются форматы UTF-16BE, UTF-32BE (начиная со старших байтов), "
                    "и UTF-16LE, UTF-32LE (начиная с младших)."sv },
    { "utf8", TermCat::SERIALIZATION,
      u8"UTF-8"sv, {},
        u8"<p>'''Unicode transformation format, 8-bit'''{{-}}формат [[pop_term:serialization|сериализации]] Юникода в цепочку байтов. "
                "Придуман в 1992 в операционной системе Plan{{_}}9 для совместимости с имеющимся интернет-ПО. "
                "Каждая [[pop_term:codepoint|кодовая позиция]] записывается одной из таких последовательностей:<br>"
                "• 0xxx.xxxx<br>"
                "• 110x.xxxx{{_}}10xx.xxxx<br>"
                "• 1110.xxxx{{_}}10xx.xxxx{{_}}10xx.xxxx<br>"
                "• 1111.0xxx{{_}}10xx.xxxx{{_}}10xx.xxxx{{_}}10xx.xxxx"
            "<p>Свойства UTF-8:<br>"
                "• [[pop_term:bom|Метка порядка байтов]] EF{{_}}BB{{_}}BF;<br>"
                "• Теоретически может записывать 2{{_}}млн кодовых позиций последовательностями до 6 байт. "
                    "С ограничениями, которые накладывает на Юникод [[pop_term:utf16|UTF-16]], не бывает последовательностей длиннее 4.<br>"
                ...
                "• Обратная совместимость: латинский текст неотличим от ASCII-текста. "
                    "Байтов FE и FF не бывает, что даёт совместимость с терминальными протоколами.<br>"
                "• Самосинхронизирующийся: если добавить любые ошибочные байты, сохранившийся текст останется читаемым. "
                    "Поиск в UTF-8 программой, не понимающей UTF-8, также не даст ложных срабатываний.<br>"
                ...
                "• Монотоный: программа, не понимающая UTF-8, отсортирует тексты в порядке кодов."sv },
    { "utf16", TermCat::SERIALIZATION,
      u8"UTF-16"sv, {},
        u8"<p>'''Unicode transformation format, 16-bit'''{{-}}формат [[pop_term:serialization|сериализации]] Юникода в цепочку двухбайтовых слов. "
            "<p>Изначально Юникод задумывался как двухбайтовая кодировка, и запись текста двухбайтовыми символами называлась UCS-2. "
                ...
                "• Символы от 10000 записываются так называемой ''суррогатной парой'':<br>"
                "{{_}}{{_}}{{_}}1101.10yy.yyyy.yyyy{{_}}1101.11yy.yyyy.yyyy,<br>"
                "{{_}}{{_}}где y=x−0x10000."
            "<p>Свойства UTF-16:<br>"
                "• [[pop_term:bom|Метка порядка байтов]] FE{{_}}FF для UTF-16BE (слова начинаются со старшего байта), "
                    "и FF{{_}}FE для UTF-16LE (слова начинаются с младшего байта);<br>"
                "• Накладывает ограничения на ёмкость Юникода{{-}}2²⁰+2¹⁶−2¹¹=1.112.064 символов.<br>"
                "• Обратная совместимость: текст в [[pop_scr:bmp|базовой плоскости]] без пользовательских символов неотличим от UCS-2.<br>"
                ...
                "• Ненадёжное опознание: без метки порядка байтов формат UTF-16 и порядок байтов определяется "
                    "по косвенным признакам;<br>"
                "• Немонотонный: программа, понимающая UCS-2, отсортирует тексты не в порядке кодов."sv },
    { "bom", TermCat::SERIALIZATION,
      u8"метка порядка байтов"sv, u8"byte order mark"sv,
        u8"<p>Символ U+FEFF. Его можно (не обязательно) ставить в начале текстового файла, чтобы указать его формат. "
                "Закодированным он будет выглядеть так:<br>"
                "• EF{{_}}BB{{_}}BF{{-}}[[pop_term:utf8|UTF-8]];<br>"
                "• FE{{_}}FF{{-}}[[pop_term:utf16|UTF-16BE]] (начиная со старшего байта);<br>"
                "• FF{{_}}FE, дальше что угодно, кроме 00{{_}}00{{-}}UTF-16LE (начиная с младшего байта);<br>"
                "• 00{{_}}00{{_}}FE{{_}}FF{{-}}[[pop_term:utf32|UTF-32BE]] (как формат межпрограммного обмена встречается крайне редко);<br>"
                "• FF{{_}}FE{{_}}00{{_}}00{{-}}UTF-32LE  (как формат межпрограммного обмена встречается крайне редко).<br>"sv },
};
