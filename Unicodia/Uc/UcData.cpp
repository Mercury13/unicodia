...
constinit const uc::Script uc::scriptInfo[] {
    ...
    { "Cakm", QFontDatabase::Any,
        ...
        u8"Говорящих на языке чакма немало и язык в списках исчезающих не числится, но письменность вытесняется [[ps:Beng|бенгальской]]. "
            ...
            "<p>Брахмийская абугида гласной по умолчанию «а:» (длинное «а») "
    ...
    { "ignorable", EcTermCat::CHAR_CLASS,
      u8"невидимый символ", u8"default-ignorable character",
        ...
        u8"Ни один шрифт не поддержит весь Юникод: дело и в простом размере (в Юникоде {{version}} символов уже {{nchars}}), "
                    ...
                    "и в метрике букв (глупо стричь под одну гребёнку [[ps:Marc|марчхен]] с высокими столбиками букв "
                        "и [[ps:pseudographics|псевдографику]], которая должна стыковаться с соседними строками)."
            "<p>Если в шрифтах не нашлось глифа, символ обычно отображаются как [[pt:tofu|тофу]]{{-}}белый квадратик. "
                "'''Но есть символы, которые запрещается так отображать.'''"
        "<p>Большинство невидимых символов являются [[pt:format|форматирующими]], но есть и непротяжённые [[pt:combining|метки]]. "
            "Невидимые символы делятся на такие категории:"
         "<p>• Символы, несущие дополнительную семантику{{-}}мягкий перенос, разъединитель графем, тэги.<br>"
            "• Пробел нулевой длины ZWSP; [[pt:bom|метка порядка байтов]], также являющаяся пробелом нулевой длины.<br>"
            "• Символы направления написания{{-}}метки слева направо и справо налево.<br>"
            "• Невидимые символы, включающие и отключающие необязательные шрифтовые механизмы ([[pt:ligature|лигатуры]] и другие){{-}}"
                "соединитель нулевой длины (ZWJ), селекторы начертания (VS), заполнители [[ps:Hang|корейских]] чамо.<br>"
            "• Символы сложного форматирования{{-}}[[pt:code|стенографии]], нот."
        "<p>'''Не являются''' невидимыми:<br>"
            "• Все имеющие графическое отображение, все [[pt:spacing|протяжённые]].<br>"
            "• [[pt:control|Управляющие]], разделители строк.<br>"
            "• Символы, задействованные в обязательных шрифтовых механизмах{{-}}например, виртуальные [[pt:virama|вирамы]]. "
    ...
    { "tofu", EcTermCat::OTHER,
      u8"тофу", u8"tofu",
        u8"[[pt:character|Глиф]], отображаемый, если символа нет в шрифте. "
            ...
            "<p>Часть символов Юникода объявлены [[pt:ignorable|невидимыми]]{{-}}не показывающимися как тофу."
