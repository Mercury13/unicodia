...
constinit const uc::Block uc::blocks[302] {
    ...
    { 0x0000, 0x007F,
            ...
            u8"<p>128 символов базовой латиницы совпадают с ASCII{{-}}американской семибитной кодировкой, ставшей основой для большинства "
                ...
                "<p>Управляющие символы, если те не задействованы в программе, часто служат [[pt:private|пользовательскими]]: "
...
constinit const uc::Term uc::terms[] {
    ...
    { "deprecated", EcTermCat::CHAR_CLASS,
      u8"запрещённый символ"sv, u8"deprecated character"sv,
        u8"<p>Символ, чьё использование настоятельно не рекомендуется."
            "<p>Одна из гарантий [[pt:stability|стабильности]] Юникода: как только символ назначен, он не сдвинется и не исчезнет. "
                "В Консорциуме Юникода сидят люди западной культуры, и они могут лишь ссылаться на исследования специалистов по редким языкам{{-}}"
                    "так случилось для пяти символов [[ps:Arab|арабского]], [[ps:Tibt|тибетского]] и [[ps:Khmr|кхмерского]]."
            "<p>Две китайские угловые скобки запретили, чтобы не путали с математическими. "
                "Также запрещены языковый тэг (внёс больше неразберихи, чем решил проблем), "
                    ...
                    "и латинская [[pt:ligature|лигатура]] '''ŉ''' (используется только как сокращение ''and'', "
                        "упразднили тип символов «буква со знаком»){{-}}итого 15 на Юникод{{_}}13.0."
            "<p>Известен случай, когда целый блок тэгов возвратили из запрета в пользование с другой целью: "
                    "[[pt:emoji|эмодзи]] флагов государственных субъектов (например, Шотландии)."
    ...
    { "precomposed", EcTermCat::CHAR_CLASS,
      u8"монолитный символ"sv, u8"precomposed character"sv,
        u8"<p>Символ и [[pt:combining|диакритическая метка]] в одном. "
            ...
            "<p>См. также [[pt:decomposition|Декомпозиция]]."sv },
    ...
    { "bmp", EcTermCat::ENCODING,
      u8"базовая многоязычная плоскость"sv, u8"BMP = Basic Multilingual Plane"sv,
        u8"<p>[[pt:plane|Плоскость]] Юникода с номером 0, содержащая символы 0000…FFFD. "
            ...
            "<p>В Юникоде 14.0 нераспределёнными остались 16 позиций, 2FE0…2FEF."sv },
    { "stability", EcTermCat::ENCODING,
      u8"гарантии стабильности"sv, u8"stability guarantees"sv,
        u8"<p>Консорциум Юникода гарантирует, что не изменит часть характеристик символов."
            ...
            "<p>'''Название символа не изменится''' (2.0+). "
                ...
                "Также не изменятся названия-синонимы, в том числе исправления (5.0+)."
            ...
            "<p>'''Свойства символа могут меняться, но не настолько, чтобы изменить его сущность''' (1.1+). "
                "Например, заглавная буква никак не может стать строчной, а [[pt:noncharacter|зарезервированный как отсутствующий]] "
                    "не вернётся в пользование. "
                "В дальнейших версиях Юникода это правило конкретизировалось, не будем уточнять."
            "<p>'''Нормативные и информационные свойства символа из баз Юникода не исчезнут''' (5.2+). "
                "Остальные могут исчезать и переименовываться, но при переименовании никогда не займут место другого."
    ...
    { "normalization", EcTermCat::ENCODING,
      u8"нормализация"sv, u8"normalization"sv,
        u8"<p>Преобразование строк для нежёсткого сравнения. "
            ...
            "<p>3. NFKD{{-}}совместимая декомпозиция (NFD + разборка [[pt:ligature|лигатур]], "
                "замена буквоподобных символов на буквы, индексов на цифры)."
            "<p>4. NFKC{{-}}совместимая композиция (после NFKD идёт '''каноническая''' сборка символов, как в NFC)."sv },
    ...
    { "casefolding", EcTermCat::ENCODING,
      u8"свёртка регистра"sv, u8"case folding"sv,
        u8"<p>Преобразование строки, используемое для (в основном) локаленезависимого сравнения строк без регистра. "
                ...
                "• Простое (без изменения длины){{-}}<big>ᾨ → ᾠ</big><br>"
                "• Полное (с раскрытием [[pt:ligature|лигатур]]){{-}}<big>ᾨ → ὠ ι</big><br>"
                "• Тюркское (специальные правила для турецкого и азербайджанского){{-}}<big>I → ı</big>"
            "<p>Обычно свёртка{{-}}это перевод в нижний регистр. "
                "Но для [[ps:Cher|чероки]] это перевод в верхний: заглавные буквы появились раньше."sv },
