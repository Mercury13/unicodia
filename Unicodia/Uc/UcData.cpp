...
constinit const uc::Term uc::terms[] {
    ...
    { "bmp", EcTermCat::ENCODING,
      u8"базовая многоязычная плоскость"sv, u8"Basic Multilingual Plane, BMP"sv,
        u8"<p>[[pt:plane|Плоскость]] Юникода с номером 0, содержащая символы 0000…FFFF. "
                "В базовой плоскости закодировны наиболее ходовые символы большинства языков, "
      ...
      u8"символ для личного пользования"sv, u8"private-use character"sv,
        u8"<p>На собственные нужды всех заинтересованных отведены 6400 символов E000…F8FF из [[pt:bmp|базовой плоскости]], "
            ...
            "<p>Договорившись, люди могут закодировать в этих диапазонах всё, что хотят, в том числе символы, не подлежащие "
                    "внесению в Юникод: товарные знаки, вымышленные алфавиты, редкие письменности, отсутствующшие в Юникоде, "
                    "редкие технические символы. "
                "Консорциум Юникода никак не отвечает за это."sv },
    ...
    { "bom", EcTermCat::SERIALIZATION,
      u8"метка порядка байтов"sv, u8"byte order mark"sv,
        u8"<p>Символ U+FEFF. Его можно (не обязательно) ставить в начале текстового файла, чтобы указать его формат. "
                ...
                "• FF{{_}}FE{{_}}00{{_}}00{{-}}UTF-32LE  (как формат текстовых файлов встречается крайне редко)."
