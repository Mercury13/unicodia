...
const uc::Category uc::categoryInfo[static_cast<int>(uc::EcCategory::NN)] {
    ...
    { UpCategory::NUMBER,       "Nd"sv,     u8"Числовой/десятичный"sv,
            ...
            u8"Изобретённая в Индии позиционная десятичная система счисления используется всем миром, однако "sv
            u8"в языках Средней и Юго-Восточной Азии цифры бывают причудливые."sv },
    ...
    { UpCategory::SYMBOL,       "Sm"sv,     u8"Символ/математической"sv,
            u8"Изначально для математики использовались несколько разных систем, наиболее известные из них\u00A0— TEX (читается «тех») "sv
            u8"и MathType. Юникод даёт надежду, что в компьютерной математике появится какая-то стандартизация\u00A0— "sv
