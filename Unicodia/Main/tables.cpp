            ...
            const QModelIndex& index, const QColor& color) const
{
    ...
    if (ch) {
        ...
        if (!color1.isValid())
            color1 = color;
        ::drawChar(painter, rect, Fsz::TABLE, 100, *ch, color1, TABLE_DRAW,
...
std::optional<QFont> VirtualCharsModel::fontAt(const QModelIndex& index) const
{
    if (auto cp = charAt(index))
        return ::fontAt(WiShowcase::EMOJI_DRAW, *cp, Fsz::TABLE, glyphSets);
    return {};
}
...
QVariant CharsModel::data(const QModelIndex& index, int role) const
{
    ...
    static constexpr QColor HILITE_DARK  { 25, 25, 112 };  // HTML MidnightBlue
    switch (role) {
        ...
        QTableView* aTable, VirtualCharsModel* aModel)
{
    ...
    acCopyVs16 = new QAction("[CopyVs16]", menu);
        ...
        connect(acCopyVs16, &QAction::triggered, table,
                [this]() {
                    emit thingCopied(uc::CopiedChannel::VS16, nullptr);
                });
...
void TableLocalMenu::addSeparator()
{
    menu->addSeparator();
}


QAction* TableLocalMenu::addCustomAction(std::string_view aLocKey)
{
    auto action = new QAction("[Custom favs]", menu);
    customActions.emplace_back(action, aLocKey);
...
void TableLocalMenu::translate()
{
    ...
    for (auto& v : customActions) {
        if (v.ac && !v.locKey.empty()) {
            v.ac->setText(loc::get(v.locKey));
        }
    }
}
...
void TableLocalMenu::popup(QWidget* widget, const QPoint& where)
{
    ...
    acCopyVs15->setVisible(charIf.hasCp() && charIf->isEmoji());
